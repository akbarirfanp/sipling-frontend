...
export function useBillsQuery() {
    ...
    const bill = computed(() => billData.value?.items || [])
    const total = computed(() => billData.value?.total || 0)
    const totalPages = computed(() => billData.value?.totalPages || 0)

    // Single bill query
    const useBillQuery = (id: MaybeRefOrGetter<string>) => {
        return useQuery({
    ...
    const generateBillMutation = useMutation({
        mutationFn: (cmd: GenerateBillCmd) => repo.generate(cmd),
        onSuccess: () => {
            // Invalidate bill list untuk refresh data
    ...
    // Prefetch bill untuk optimistic loading
    const prefetchBill = (id: string) => {
    ...
    return {
        ...
        // Loading states
        isLoadingBill,
        isCreatingBill: generateBillMutation.isPending,
...
export function useBills() {
    ...
    return {
        bills: query.bill,
        isLoadingBill: query.isLoadingBill,
        billsError: computed(() => query.billsError.value?.message || null),
        total: query.total,
        ...
        search: query.search,
        status: query.status,
        refreshBill: query.refreshBill,
        generateBill: query.generateBill,
    }
}
