...
const yearList = computed(() => {
  const minYear = props.minValue ? props.minValue.year : currentYear - 100
  const maxYear = props.maxValue ? props.maxValue.year : currentYear
...
<template>
  ...
  >
    <CalendarHeader>
      <CalendarPrevButton />
      <CalendarHeading
        class="mx-2 flex gap-2 w-full items-center justify-between"
      >
        <!-- Month Selector -->
        <Select
    ...
    <!-- Calendar body -->
    <div
      class="pt-4 flex flex-col space-y-4 sm:flex-row sm:gap-x-4 sm:gap-y-0"
    >
